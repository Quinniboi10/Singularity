...
namespace chess::movegen {
    ...
    constexpr int diagonal_of(Square s) {
        return 7 + s.rank() - s.file();
    }
    constexpr int antidiagonal_of(Square s) {
        return s.rank() + int(s.file());
    }
    ...
    BitBoard sliding_attacks(Square square, BitBoard occ, BitBoard mask) {
        return (((mask & occ) - square.as_bb() * 2) ^ reverse(reverse(mask & occ) - reverse(square.as_bb()) * 2)) & mask;
    }
    ...
    BitBoard get_rook_attacks_for_init(Square square, BitBoard occ) {
        return sliding_attacks(square, occ, MASK_FILE[square.file()]) | sliding_attacks(square, occ, MASK_RANK[square.rank()]);
    }
    ...
    BitBoard get_rook_attacks(Square sq, BitBoard occ) {
        return ROOK_ATTACKS[sq.sq][((occ & ROOK_ATTACK_MASKS[sq.sq]).as_u64() * ROOK_MAGICS[sq.sq]) >> ROOK_ATTACK_SHIFTS[sq.sq]];
    }
    ...
    BitBoard get_bishop_attacks(Square sq, BitBoard occ) {
        return BISHOP_ATTACKS[sq.sq][((occ & BISHOP_ATTACK_MASKS[sq.sq]) * BISHOP_MAGICS[sq.sq]).as_u64() >> BISHOP_ATTACK_SHIFTS[sq.sq]];
    }
